...
def _make_efficientnet(model):
    # This function is adapted for newer timm versions where `model.act1` might not exist.
    # We use nn.SiLU directly, which is the standard activation for EfficientNet.
    activation_layer = getattr(model, 'act1', nn.SiLU(inplace=True))

    pretrained = nn.Module()
    pretrained.layer0 = nn.Sequential(model.conv_stem, model.bn1, activation_layer, *model.blocks[0:2])
