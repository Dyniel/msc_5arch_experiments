...
from .graph_hook import GraphHook
...
class StyleGAN2Loss(Loss):
    def __init__(self, device, G, D, augment_pipe=None, r1_gamma=10, style_mixing_prob=0, pl_weight=0,
        ...
        self.graph_hook = None
        if graph_kwargs.get('graph', False):
            self.graph_hook = GraphHook(batch_size=batch_size, **graph_kwargs)
        ...
        if phase in ['Gmain', 'Gboth']:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                ...
                if self.graph_hook is not None:
                    loss_G_graph, graph_stats = self.graph_hook(gen_img, cur_nimg)
                    if loss_G_graph is not None:
                        for name, value in graph_stats.items():
                            training_stats.report(name, value)
                        loss_Gmain = loss_Gmain + loss_G_graph
